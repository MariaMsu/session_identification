...
df = df.withColumn("timestamp", unix_timestamp("timestamp", "yyyy-MM-dd HH:mm:ss").cast("long"))
# df = df.withColumn("timestamp", unix_timestamp("timestamp", "yyyy-MM-dd HH:mm:ss").cast("timestamp"))

df = df.withColumn("date", date_format(from_unixtime("timestamp"), "yyyy-MM-dd"))
...
window_spec = Window.partitionBy("user_id", "product_code", "date").orderBy("timestamp")  # todo delete date
...
df = df.withColumn("time_diff", when(df.prev_timestamp.isNull(), 0).otherwise((df.timestamp - df.prev_timestamp) / 60))
...
# df = df.withColumn("session_id", F.first('timestamp').filter('new_session == 1').over(window_spec))
# df = df.withColumn("session_id", sum("new_session").over(window_spec))
# df = df.withColumn("session_id", concat_ws("#", "user_id", "product_code", date_format("timestamp", "yyyy-MM-dd"), "session_id"))
